...
namespace RestaurantData
{
    public class clsCoinsTransactionsData
    {
        ...
        public static decimal GetSavedAmountFromCoins(int UserID)
        {
            using (var connection = new SqlConnection(clsDataSettings.ConnectionString))
            using (var command = new SqlCommand("SP_GetSavedAmountFromCoins", connection))
            {
                command.CommandType = CommandType.StoredProcedure;

                command.Parameters.AddWithValue("@UserID", UserID);

                var SavedAmountFromCoins = new SqlParameter("@SavedAmountFromCoins", SqlDbType.Decimal)
                {
                    Direction = ParameterDirection.Output
                };
                command.Parameters.Add(SavedAmountFromCoins);

                connection.Open();
                command.ExecuteNonQuery();

                return (decimal)SavedAmountFromCoins.Value;
            }
        }
    }
}
